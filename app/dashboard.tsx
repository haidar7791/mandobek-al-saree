  ...
  return (
    <View style={formStyles.addressGroup}>
      ...
      <FormField
        ...
        onChangeText={(v) => onChange("governorate", v)}
        icon={<Feather name="map-pin" size={15} color={C.textMuted} />}
      />
      <FormField
        label="الحي"
...
function AddOrderModal({
  ...
  const [merchantAddress, setMerchantAddress] = useState<AddressFields>(
    emptyAddress()
  );
  const [merchantPhone, setMerchantPhone] = useState("");
  const [customerAddress, setCustomerAddress] = useState<AddressFields>(
    emptyAddress()
  );
  ...
  const reset = () => {
    setProductName("");
    setMerchantAddress(emptyAddress());
    setMerchantPhone("");
  ...
  const handleCreate = async () => {
    ...
    if (
      !merchantAddress.governorate ||
      !merchantAddress.neighborhood ||
      !merchantAddress.street
    ) {
      Alert.alert("خطأ", "يرجى إكمال عنوان التاجر");
      return;
    }
    if (!merchantPhone.trim()) {
      Alert.alert("خطأ", "يرجى إدخال هاتف التاجر");
      return;
    }
    if (
      ...
      Alert.alert("خطأ", "يرجى إكمال عنوان الزبون");
      return;
    }
    if (!customerPhone.trim()) {
      Alert.alert("خطأ", "يرجى إدخال هاتف الزبون");
      return;
    }
    const pp = parseFloat(productPrice);
    const dp = parseFloat(deliveryPrice);
    if (!productPrice || isNaN(pp) || pp <= 0) {
      Alert.alert("خطأ", "يرجى إدخال سعر المنتج");
      return;
    }
    if (!deliveryPrice || isNaN(dp) || dp < 0) {
      Alert.alert("خطأ", "يرجى إدخال سعر التوصيل");
      return;
    }
      ...
      >
        ...
        <View style={[modalStyles.sheet, { maxHeight: "92%" }]}>
          ...
          >
            <FormField
              label="اسم المنتج"
              placeholder="مثال: جوال سامسونج A54"
              value={productName}
            ...
            <View style={formStyles.codeNote}>
              ...
              <Text style={formStyles.codeNoteText}>
                سيتم توليد رمز سري تلقائياً عند إنشاء الطلب يُستخدم لتأكيد
                التوصيل أو الإرجاع
              </Text>
            </View>

            <Pressable
              style={[formStyles.submitBtn, loading && { opacity: 0.6 }]}
        ...
        <View style={modalStyles.codeCard}>
          ...
          <View style={modalStyles.codeBox}>
            {code.split("").map((char, i) => (
              <View key={i} style={modalStyles.codeChar}>
                <Text style={modalStyles.codeCharText}>{char}</Text>
              </View>
            ))}
          </View>
...
function EnterCodeModal({
  ...
  const handleConfirm = () => {
    if (code.trim().length < 4) {
      Alert.alert("خطأ", "يرجى إدخال الرمز السري الصحيح");
      return;
    }
    ...
    >
      ...
      >
        ...
        <View style={modalStyles.enterCodeSheet}>
          ...
          >
            <Feather
              name={isDelivery ? "check-circle" : "rotate-ccw"}
              size={28}
              color={isDelivery ? C.success : "#8B5CF6"}
            />
          </View>
          <Text style={modalStyles.enterCodeTitle}>
            {isDelivery ? "تأكيد التوصيل" : "تأكيد الإرجاع"}
          </Text>
          ...
          <Pressable
            style={[
              ...
              {
                backgroundColor: isDelivery ? C.success : "#8B5CF6",
              },
            ]}
    ...
    <View style={cardStyles.card}>
      <View style={cardStyles.cardHeader}>
        <View style={[cardStyles.statusBadge, { backgroundColor: st.bg }]}>
          ...
          <Text style={[cardStyles.statusText, { color: st.color }]}>
            {st.label}
          </Text>
        </View>
      ...
      <View style={cardStyles.priceRow}>
        <View style={cardStyles.priceItem}>
          <Text style={cardStyles.priceLabel}>التوصيل</Text>
          <Text style={[cardStyles.priceValue, { color: "#3B82F6" }]}>
            {order.deliveryPrice} ريال
          </Text>
        </View>
        <View style={cardStyles.priceDivider} />
        <View style={cardStyles.priceItem}>
          <Text style={cardStyles.priceLabel}>المنتج</Text>
          <Text style={[cardStyles.priceValue, { color: C.text }]}>
            {order.productPrice} ريال
          </Text>
        </View>
      </View>
      ...
      <View style={cardStyles.addrRow}>
        ...
        <View style={cardStyles.addrItem}>
          ...
          <Text style={cardStyles.addrText} numberOfLines={1}>
            {order.merchantAddress.governorate} ·{" "}
            {order.merchantAddress.neighborhood}
          </Text>
        </View>
      </View>
        ...
        <View style={cardStyles.codeRow}>
          ...
          <View style={cardStyles.codePill}>
            {order.uniqueCode.split("").map((c, i) => (
              <Text key={i} style={cardStyles.codeChar}>
                {c}
              </Text>
            ))}
          </View>
      ...
      {(isMyDelivery || isMerchant) && onAction && (
        <Pressable style={cardStyles.actionArea} onPress={onAction}>
          <Text style={cardStyles.actionText}>
    ...
    <View style={cardStyles.card}>
      ...
      <View style={cardStyles.priceRow}>
        <View style={cardStyles.priceItem}>
          ...
          <Text style={[cardStyles.priceValue, { color: C.success }]}>
            +{order.deliveryPrice} ريال
          </Text>
        </View>
        <View style={cardStyles.priceDivider} />
        <View style={cardStyles.priceItem}>
          ...
          <Text style={[cardStyles.priceValue, { color: C.danger }]}>
            -{order.productPrice} ريال
          </Text>
        </View>
      </View>
      ...
      <View style={cardStyles.addrRow}>
        <View style={cardStyles.addrItem}>
          ...
          <Text style={cardStyles.addrText} numberOfLines={1}>
            التوصيل: {order.customerAddress.governorate} ·{" "}
            {order.customerAddress.neighborhood}
          </Text>
        </View>
        <View style={cardStyles.addrItem}>
          <Feather name="map-pin" size={12} color={C.textMuted} />
          <Text style={cardStyles.addrText} numberOfLines={1}>
            الاستلام: {order.merchantAddress.governorate} ·{" "}
            {order.merchantAddress.neighborhood}
          </Text>
        </View>
      </View>
      ...
      {!canAfford && (
        <View style={cardStyles.noBalanceWarn}>
          ...
          <Text style={cardStyles.noBalanceText}>
            رصيدك غير كافٍ لقبول هذا الطلب ({order.productPrice} ريال ضمان)
          </Text>
        </View>
      )}
      ...
      >
        <LinearGradient
          colors={
            canAfford ? [C.accent, C.accentLight] : [C.border, C.border]
          }
        ...
        >
          ...
          <Text
            style={[
              cardStyles.acceptText,
              !canAfford && { color: C.textMuted },
            ]}
          >
            قبول الطلب
          </Text>
...
export default function DashboardScreen() {
  ...
  const handleAcceptOrder = async (order: Order) => {
    Alert.alert(
      "تأكيد قبول الطلب",
      `سيتم خصم ${order.productPrice} ريال من رصيدك كضمان للمنتج. هل تريد المتابعة؟`,
      [
          ...
          onPress: async () => {
            try {
              ...
              const updated = allOrders.map((o) =>
                o.id === order.id
                  ? { ...o, status: "in_delivery" as OrderStatus, acceptedBy: currentUser }
                  : o
              );
              ...
              Alert.alert(
                "تم قبول الطلب",
                `تم قبول طلب "${order.productName}" بنجاح. تم خصم ${order.productPrice} ريال من رصيدك.`
              );
            } catch {
  ...
  const handleConfirmCode = async (enteredCode: string) => {
    ...
    try {
      const newStatus: OrderStatus =
        mode === "deliver" ? "delivered" : "returned";
      const updated = allOrders.map((o) =>
      ...
      if (mode === "deliver") {
        ...
        Alert.alert(
          "تم التوصيل",
          `تم تأكيد التوصيل بنجاح! تمت إضافة ${order.productPrice + order.deliveryPrice} ريال إلى رصيدك.`
        );
      } else {
        ...
        Alert.alert(
          "تم الإرجاع",
          `تم تأكيد الإرجاع. تمت إعادة ${order.productPrice} ريال إلى رصيدك.`
        );
      }
    } catch {
  ...
  const marketOrders = allOrders.filter(
    (o) => o.merchantId !== currentUser && o.status === "pending"
  );
  const myDeliveries = allOrders.filter(
    (o) => o.acceptedBy === currentUser
  );
  ...
  const emptyMessages: Record<ActiveTab, { title: string; sub: string }> = {
    my_orders: {
      ...
      sub: "أنشئ طلب توصيل جديد للبدء",
    },
    market: {
      title: "السوق فارغ حالياً",
      sub: "لا توجد طلبات متاحة للقبول",
    },
    my_deliveries: {
      title: "لا توجد توصيلات",
      sub: "اذهب إلى السوق لقبول طلبات التوصيل",
    },
  };
  ...
  return (
    <View style={styles.root}>
              ...
              <OrderCard
                ...
                onAction={
                  ...
                  item.status === "in_delivery"
                    ? () => {
                        Alert.alert(
                          `إدارة: ${item.productName}`,
                          "اختر الإجراء المناسب",
                          [
                            { text: "إلغاء", style: "cancel" },
                            {
                              text: "تم الإرجاع للتاجر",
                              onPress: () =>
                                setCodeEntry({
                                  orderId: item.id,
                                  mode: "return",
      ...
      {newOrderCode && (
        <CodeModal
          visible={!!newOrderCode}
          code={newOrderCode.code}
          productName={newOrderCode.name}
          onClose={() => setNewOrderCode(null)}
        />
      )}
