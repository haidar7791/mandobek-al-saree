...
import {
  ...
  type ChatReply,
  subscribeToMessages,
  loadOlderMessages as fetchOlderMessages,
  markMessagesRead,
...
export default function ChatRoom({
  ...
  // true = المستخدم في آخر المحادثة ويمكن تحريكه تلقائياً
  // false = المستخدم يتصفح الرسائل القديمة ولا يجب إعادته للأسفل
  ...
  const loadOlderChatMessages = useCallback(async () => {
    ...
    try {
      ...
      console.error("load older chat messages failed:", error);
    } finally {
      setLoadingOlderMessages(false);
    }
  }, [
  ...
  const createReplySwipeResponder = (item: ChatMessage) =>
    PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (_, gesture) => {
        // نعطي السحب الأفقي أولوية حتى مع ظهور الكيبورد أو وجود Pressable داخل الرسالة.
    ...
    >
      ...
      <FlatList
        ...
        onScroll={({ nativeEvent }) => {
          ...
          // مقدار المسافة بين أسفل المحتوى وأسفل الشاشة
          const distanceFromBottom =
            contentSize.height -
            (contentOffset.y + layoutMeasurement.height);

          // إذا كان المستخدم قريباً من آخر رسالة،
          // يسمح للنظام بالانتقال تلقائياً للرسائل الجديدة.
          isNearBottomRef.current = distanceFromBottom <= 80;

          // تحميل الرسائل الأقدم فقط عند السحب إلى أعلى المحادثة.
          if (
            contentOffset.y <= 80 &&
        ...
        onContentSizeChange={() => {
          if (
            ...
            // بعد ذلك:
            // لا نتحرك تلقائياً إلا إذا كان المستخدم أصلاً
            // في نهاية المحادثة.
      ...
      {isRecording ? (
        ...
        <View style={[styles.inputBar, { paddingBottom: bottomPad + 8 }]}>
          ...
          <View style={styles.composerShell}>
            ...
            <TextInput
              ref={inputRef}
              style={[styles.textInput, replyingTo && styles.textInputReplying]}
              placeholder={replyingTo ? "قم بالرد على الرسالة" : "اكتب رسالتك..."}
...
const styles = StyleSheet.create({
  ...
  headerName: { fontSize: 16, fontFamily: undefined, color: "#FFF", textAlign: "right" },
  headerSub: { fontSize: 12, fontFamily: undefined },
  ...
  headerAvatarText: { fontSize: 18, fontFamily: undefined, color: C.accent },
  ...
  replyPreviewName: { fontSize: 10, fontFamily: undefined, textAlign: "right" },
  replyPreviewText: { fontSize: 10, fontFamily: undefined, textAlign: "right" },
  ...
  messageActionText: { fontSize: 11, fontFamily: undefined, color: C.primary },
  heartAction: { fontSize: 18, lineHeight: 20 },
  likeCount: { fontSize: 10, fontFamily: undefined, color: C.textMuted },
  ...
  groupSenderAvatarText: { fontSize: 10, fontFamily: undefined, color: C.accent },
  groupSenderName: { fontSize: 10, fontFamily: undefined, color: C.accent, textAlign: "right" },
  adminLabel: {
  ...
  msgTime: { fontSize: 10, fontFamily: undefined },
  ...
  locationTitle: { fontSize: 14, fontFamily: undefined, color: C.text },
  locationSub: { fontSize: 10, fontFamily: undefined, color: C.textMuted },
  optionBackdrop: {
  ...
  locationMenuTitle: { fontSize: 16, fontFamily: undefined, color: C.text, textAlign: "right", marginBottom: 10 },
  locationOption: {
  ...
  locationOptionTitle: { fontSize: 13, fontFamily: undefined, color: C.text },
  locationOptionSub: { fontSize: 10, fontFamily: undefined, color: C.textMuted, textAlign: "right" },
  ...
  audioDuration: { fontSize: 13, fontFamily: undefined, flex: 1 },
  emptyChat: { alignItems: "center", paddingTop: 60, gap: 10 },
  emptyChatText: { fontSize: 14, fontFamily: undefined, color: C.textMuted },
  inputBar: {
  ...
  orderCardTitle: { fontSize: 15, fontFamily: undefined, textAlign: "right", lineHeight: 23 },
  orderDetailGrid: { gap: 3 },
  orderDetailText: { fontSize: 11, fontFamily: undefined, textAlign: "right", lineHeight: 18 },
  ...
  orderActionText: { fontSize: 10, fontFamily: undefined },
  // ── Card bubble ──
  ...
  cardBubbleDetail: { fontSize: 11, fontFamily: undefined, textAlign: "right" },
  ...
  cardBubbleBtnText: { fontSize: 13, fontFamily: undefined },

  deletedRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  deletedText: { fontSize: 13, fontFamily: undefined, fontStyle: "italic" },
  composerShell: { flex: 1, backgroundColor: C.inputBg, borderRadius: 16, overflow: "hidden", minHeight: 44 },

  replyComposer: {
    
    transform: [{ translateY: -18 }],flexDirection: "row",
    alignItems: "center",
    minHeight: 52,
    marginHorizontal: 8,
    marginTop: 6,
  ...
  pendingLabel: { flex: 1, fontSize: 12, fontFamily: undefined, color: C.textMuted, textAlign: "right" },
  pendingCancelBtn: {
    width: 30,
    height: 30,
    borderRadius: 15,
