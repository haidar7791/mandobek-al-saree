...
**How to apply:** Before treating a new media flow as complete, confirm the selected Firebase project and bucket, publish the reviewed Storage rules without making the bucket public, then smoke-test upload, download, Firestore write, and playback with an authenticated account. Use the protected `FIREBASE_SERVICE_ACCOUNT` secret for publication; the checked-in service-account key has an invalid JWT signature.