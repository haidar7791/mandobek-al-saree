...
export const subscribeToMessages = (
  ...
  const q = query(
    ...
    limitToLast(CHAT_MESSAGES_PAGE_SIZE)
  );
...
export const loadOlderMessages = async (
  ...
  return {
    messages: msgs,
    hasMore: msgs.length === CHAT_MESSAGES_PAGE_SIZE,
  };
};
...
/**
 * Resolve the real accounts followed by a user.
 *
 * Follow source of truth:
 * users/{followedUserId}/followers/{followerId}
 *
 * We inspect the existing followers subcollections and select
 * documents whose follower document id equals profileId.
 * This preserves existing follow data without introducing
 * a separate/duplicated following counter.
 */
export const getFollowingProfiles = async (
  ...
  // followers documents are stored as:
  // users/{followedUserId}/followers/{followerId}
  //
  // The followerId is the document ID, not a field inside the document.
  // Firestore collectionGroup(documentId()) requires a full document path,
  // so querying documentId() == profileId is invalid here.
  //
  // Keep the existing database structure and resolve the matching follower
  // documents locally instead of changing follow/unfollow data.
  const snap = await getDocs(collectionGroup(db, "followers"));

  const matchingFollowers = snap.docs.filter(
    (followerDoc) => followerDoc.id === profileId
  );

  const profiles: Array<FollowerProfile | null> = await Promise.all(
    matchingFollowers.map(async (followerDoc) => {
      const followedUserRef = followerDoc.ref.parent.parent;
      const followedUserId = followedUserRef?.id;

      if (!followedUserId) return null;

      const followedUser = await getUserProfile(followedUserId);

      if (!followedUser) return null;

      return {
        id: followedUserId,
        name: followedUser.name || "مستخدم",
        photoUri: followedUser.photoUri || null,
  ...
  return profiles
    ...
    .sort((a, b) => {
      const aTime =
        a.followedAt?.toMillis?.() ??
        (typeof a.followedAt?.seconds === "number"
          ? a.followedAt.seconds * 1000
          : 0);

      const bTime =
        b.followedAt?.toMillis?.() ??
        (typeof b.followedAt?.seconds === "number"
          ? b.followedAt.seconds * 1000
          : 0);
...
/* ============================================================
   FOOD ORDERS — طلبات المطاعم
   مستقل عن ServiceRequest و ProductOrder
   ============================================================ */
...
export type FoodOrderPayment = "cash" | "wallet";
...
export const createFoodOrder = async (input: {
  restaurantId: string;
  restaurantName: string;
  items: FoodOrderItem[];
  total: number;
...
export const subscribeToCustomerFoodOrders = (
  userId: string,
  ...
  return onSnapshot(
    ...
    (error) => {
      console.error("subscribeToCustomerFoodOrders error:", error);
      onError?.(error);
    },
  );
};

export const subscribeToRestaurantFoodOrders = (
  restaurantId: string,
  callback: (orders: FoodOrder[]) => void,
  onError?: (error: Error) => void,
): Unsubscribe => {
  ...
  return onSnapshot(
    ...
    (error) => {
      console.error("subscribeToRestaurantFoodOrders error:", error);
      onError?.(error);
    },
  );
};
...
export const getFoodOrder = async (
  ...
  return {
    id: snap.id,
    ...(snap.data() as Omit<FoodOrder, "id">),
  };
};
...
export const updateFoodOrderStatus = async (
  ...
  if (!snap.exists()) throw new Error("FOOD_ORDER_NOT_FOUND");

  const data = snap.data() as Omit<FoodOrder, "id">;

  const isRestaurant = data.restaurantId === user.uid;
  ...
  if (isRestaurant && data.customerId) {
    ...
    await createActivityNotification({
      ...
      body:
        status === "accepted"
          ? `تم قبول طلبك من ${data.restaurantName}`
          : status === "rejected"
            ? `تم رفض طلبك من ${data.restaurantName}`
